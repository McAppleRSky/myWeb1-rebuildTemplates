...
var OnesReadystatechangeCount, Photo, albumHandler, curPhotoViewHeight, curPhotoViewWidth, currentPhotoView, file, fileElem, fillListAndPhotosAndAlbumHandler, fillListFromFileAndfillPhotosAndAlbumHandler, fillListFromHttp, fillPhotos, formated, full, height, i, imgListPath, j, k, listCount, listFilename, listfFilename, lists, myConsole, photos, reader, readerIndex, setPhotoViewLayer, title, width;
...
fillListFromFileAndfillPhotosAndAlbumHandler = function(event) {
  ...
  while (i < files.length) {
    if (files[i].name === listFilename && full === false) {
      ...
      if (files[i].name === listfFilename && formated === false) {
        reader[++readerIndex] = new FileReader();
        reader[readerIndex].onload = function(e) {
          var str;
          str = this.result.split('\n');
...
fillPhotos = function() {
  var curPhotoParam, fileInList, item, l, len, len1, m, ref, ref1;
  curPhotoParam = null;
  fileInList = null;
  ref = lists[formated];
...
albumHandler = function() {
  var anchEl, cellCount, figEl, figcapEl, fileName, imgEl, newCell, newRow, photoTable, photoWin, tableWidthPercent, winImg;
  photoTable = null;
  photoWin = null;
  winImg = null;
  cellCount = 4;
